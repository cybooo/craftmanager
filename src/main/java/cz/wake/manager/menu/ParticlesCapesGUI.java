...
public class ParticlesCapesGUI implements InventoryProvider {
    ...
    public void init(Player player, InventoryContents contents) {
        contents.fillRow(0, ClickableItem.of(new ItemBuilder(Material.STAINED_GLASS_PANE).setDurability((short)4).setName("§a ").build(), item -> {}));
        contents.fillRow(5, ClickableItem.of(new ItemBuilder(Material.STAINED_GLASS_PANE).setDurability((short)4).setName("§a ").build(), item -> {}));
        ...
        if (player.hasPermission("craftmanager.particles.cape.christmas")) {
            ...
            contents.set(2, 2, ClickableItem.of(new ItemBuilder(Material.RED_MUSHROOM).setName("§4§lChristmas Cape").setLore("§7Lze získat pomocí Vánoční Bundle 2019").build(), e -> { }));
        }
